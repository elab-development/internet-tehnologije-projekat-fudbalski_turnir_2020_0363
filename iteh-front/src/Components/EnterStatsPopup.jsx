...
const EnterStatsPopup = ({ id, onClose}) => {
  ...
  useEffect(() => {
    const fetchPlayerStats = async () => {
      try {
        ...
        setTeam1Name(data.home_team.name);
        setTeam2Name(data.away_team.name);
        ...
        setTeam1Stats({
          id:homeTeamStats.map(player => player.id),
          names: homeTeamStats.map(player => player.name),
          goals: homeTeamStats.map(player => player.goals),
          assists: homeTeamStats.map(player => player.assists),
          shotsOnTarget: homeTeamStats.map(player => player.shotsOnTarget),
          shotsOffTarget: homeTeamStats.map(player => player.shotsOffTarget),
          yellowCards: homeTeamStats.map(player => player.yellowCards),
          redCards: homeTeamStats.map(player => player.redCards),
          possession: data.game_stats.posed_lopte_domacin || 0 
        });

        setTeam2Stats({
          id:awayTeamStats.map(player => player.id),
          names: awayTeamStats.map(player => player.name),
          goals: awayTeamStats.map(player => player.goals),
          assists: awayTeamStats.map(player => player.assists),
          shotsOnTarget: awayTeamStats.map(player => player.shotsOnTarget),
          shotsOffTarget: awayTeamStats.map(player => player.shotsOffTarget),
          yellowCards: awayTeamStats.map(player => player.yellowCards),
          redCards: awayTeamStats.map(player => player.redCards),
          possession: data.game_stats.posed_lopte_gost || 0 
        });
      } catch (error) {
  ...
  if (showPlayerStats) {
    return <PlayerStatsPopup id={id} onClose={onClose} />;
  }
  ...
  const handleChange = (team, index, stat, value) => {
    ...
    setStats(prev => {
      const newStats = {
        ...prev,
        [stat]: stat === 'possession' ? value : prev[stat].map((val, i) => (i === index ? value : val))
      };
  ...
  const handleSave = async () => {
    try {
      ...
      const response =await axios.put(`http://localhost:8000/api/games/${id}`, updatedStats, {
        headers: {
          Authorization: "Bearer " + window.sessionStorage.getItem('auth_token'),
        }
      });
  ...
  const renderStatsTable = (teamStats, teamNumber) => (
    <div className="stats-table">
      ...
      {['goals', 'assists', 'shotsOnTarget', 'shotsOffTarget', 'yellowCards', 'redCards'].map((stat, idx) => (
        <div key={idx} className="stats-row">
          ...
          {teamStats[stat].map((value, i) => (
            <input
              ...
              value={value}
              min="0"
              onChange={(e) => handleChange(teamNumber, i, stat, Number(e.target.value))}
      ...
      <div className="stats-row">
        ...
        <input
          ...
          min="0"
          max="100"
          onChange={(e) => handleChange(teamNumber, null, 'possession', Number(e.target.value))}
          onKeyDown={(e) => e.preventDefault()}
        />
      </div>
    </div>
  ...
  const getStatLabel = (stat) => {
    switch (stat) {
      case 'goals': return 'Golovi';
      case 'assists': return 'Asistencije';
      case 'shotsOnTarget': return 'Šutevi u okvir';
      case 'shotsOffTarget': return 'Šutevi van okvira';
      case 'yellowCards': return 'Žuti kartoni';
      case 'redCards': return 'Crveni kartoni';
      default: return '';
    }
  };
    ...
    <div className="popup-overlay">
      <div className="popup-content">
        ...
        <div className="popup-buttons">
          ...
          <button className="update-btn" onClick={handleSave}>Sačuvaj</button>
        </div>
