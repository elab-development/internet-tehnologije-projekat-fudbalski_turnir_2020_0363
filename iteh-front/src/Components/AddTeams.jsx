...
const AddTeams = () => {
  ...
  const [teams, setTeams] = useState(() => {
    const initialTeams = Array(location.state?.numTeams || 4).fill({ id: null, name: '', players: [] });
  ...
  const [tournamentName, setTournamentName] = useState(location.state?.name || 'Unknown Tournament');
  const [placeOfPlaying, setPlaceOfPlaying] = useState(location.state?.place || 'Unknown Place');
  ...
  useEffect(() => {
    if (location.state?.newTeam !== undefined && location.state?.rowIndex !== undefined) {
  ...
  const handleCreateTournament = async () => {
    if (teams.some(team => !team.name || team.players.length < 5)) {
      alert('Ubacite sve timove i dodajte 5 igrača pre nego što kreirate turnir.');
      return;
    }
    ...
    for (const team of teams) {
      if (teamNames.has(team.name)) {
        alert('Postoje timovi sa istim imenom. Molimo vas da ih uklonite pre nego što kreirate turnir.');
        return;
      }
    ...
    try {
      ...
      if (response.data.success === true) {
        ...
        navigate('/tournaments',{ state: { role: window.sessionStorage.getItem('role') } });
      } else {
  ...
  return (
    <div className="add-teams-container">
      ...
      <div className="tournament-info">
        <p><strong>NAZIV TURNIRA:</strong> {tournamentName}</p>
        {logo && (
          <div className="logo-container">
            <img src={logo} alt="Tournament Logo" className="logo-image" />
          </div>
        )}
        <p><strong>MESTO IGRANJA:</strong> {placeOfPlaying}</p>
      </div>
